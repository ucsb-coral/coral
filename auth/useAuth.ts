...
import {
  AuthStackNavigatorScreens,
  authStackNavigate,
} from '../src/navigation/navigators/StackNavigator';
...
export const enterAuthFlow = () =>
  store.dispatch(setAuthStateAction({authState: 'AUTHENTICATING'}));
...
export default function useAuth() {
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const result = (await WebBrowser.openAuthSessionAsync(
        authlink,
        redirectUri,
      )) as WebBrowser.WebBrowserRedirectResult | {type: 'cancel'};

      if (result?.type === 'cancel') throw new Error('User cancelled');
      //   if (platform === 'ios') {
    ...
    switch (authState) {
      ...
      case 'AUTHENTICATING': {
        Linking.addEventListener('url', ({url}) => {
          store.dispatch(setAuthStateAction({authState: 'LINK_RECIEVED'}));
          handleSignIn(url);
        });
  ...
  useEffect(() => {
    ...
    if (appState === 'active' && authState === 'AUTHENTICATING') {
      abortSignIn();
    }
  }, [appState]);
